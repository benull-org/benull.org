...
const CreateOrJoinRoom: React.FC = () => {
  ...
  const [targetUsername, setTargetUsername] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get('dm') || '';
  });
  const [isJoining, setIsJoining] = useState(false);
  const [mode, setMode] = useState<'ROOM' | 'DIRECT'>(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get('dm') ? 'DIRECT' : 'ROOM';
  });
  const [autoProcessed, setAutoProcessed] = useState(false);

  // Auto-join logic based on URL parameters
  React.useEffect(() => {
    // Only run if we have an identity and haven't processed yet
    if (!userIdentity || autoProcessed) return;

    const params = new URLSearchParams(window.location.search);
    const roomParam = params.get('room');
    const dmParam = params.get('dm');

    if (!roomParam && !dmParam) {
      setAutoProcessed(true);
      return;
    }

    const processAutoJoin = async () => {
      console.log("Processing auto-join...", { roomParam, dmParam });
      ...
      if (roomParam) {
        ...
        setIsJoining(true);
        try {
          const isOnline = await checkUserOnline(dmParam);
          if (isOnline) {
            await startDirectChat(dmParam);
          } else {
            console.warn(`User ${dmParam} is currently offline.`);
          }
        } catch (e) {
          console.error("Auto-join DM failed:", e);
        } finally {
          setIsJoining(false);
          const newUrl = window.location.pathname;
          window.history.replaceState({}, '', newUrl);
    ...
    processAutoJoin();
  }, [userIdentity, autoProcessed, joinRoom, startDirectChat, checkUserOnline]);
        ...
        <div className="mb-8 p-4 bg-[#050505] rounded-2xl border border-[#333] flex items-center justify-between group relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => {
                  if (userIdentity?.username) {
                    const url = `${window.location.origin}/?dm=${encodeURIComponent(userIdentity.username)}`;
                    navigator.clipboard.writeText(url);
                    // Visual feedback could be added here, e.g., a toast or temporary icon change
                    // For now, let's use a simple alert or console log if we don't have a toast system
                    // But to make it "wow", let's change the icon temporarily.
                    const btn = document.getElementById('share-btn-icon');
                    if (btn) {
                      const original = btn.innerHTML;
                      btn.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />';
              ...
              >
                <svg id="share-btn-icon" xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
